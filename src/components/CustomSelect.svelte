...
<div class="relative inline-block text-left">
    <div>
        <button type="button" onclick={toggleDropdown} class="{isOpen ? 'rounded-t-md' : 'rounded-md'} transition-all duration-200 inline-flex items-center justify-between w-full shadow-sm px-2 py-2 bg-[--bg-primary] text-sm font-medium text-[--text-primary]" aria-haspopup="true" aria-expanded={isOpen}>
    ...
    <div id="dropdown" class="absolute z-10 w-full rounded-b-md bg-[--bg-primary] shadow-lg" transition:slide={{duration: 100}}>
        <div class="py-1 divide-y" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
            {#each options.filter(opt => opt !== selected) as option}
                <button onclick={() => selectOption(option)} class="block w-full text-left px-2 py-1 text-sm text-[--text-secondary]">
                    {option}
                </button>
            {/each}
        </div>
